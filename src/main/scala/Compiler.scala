...
object Compiler {
  ...
  def ioCompiler: Action ~> Task = new (Action ~> Task) {
    override def apply[A](fa: Action[A]): Task[A] = fa match {
      case PlayRound(s1, s2) => Task.delay(Rules.winner(s1, s2))
      case AskForShape => Task.delay { print("Your shape"); } flatMap { _ ⇒ inShape }
      case AskAIForShape => Task.delay { val shape = Strategies.random; println(s" against $shape"); shape }
